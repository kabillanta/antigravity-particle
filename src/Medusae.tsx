...
export interface MedusaeConfig {
  ...
  halo: {
    ...
    outerOscJitterStrength: number;
    outerOscJitterSpeed: number;
    radiusBase: number;
...
// Helper to make deep partial types
type DeepPartial<T> = T extends object ? {
    [P in keyof T]?: DeepPartial<T[P]>;
} : T;

const MEDUSAE_DEFAULTS: MedusaeConfig = {
  cursor: {
    radius: 0.065,
    strength: 3,
    dragFactor: 0.015,
...
const mergeConfig = (config?: DeepPartial<MedusaeConfig>): MedusaeConfig => ({
...
const Particles = ({ config }: { config?: DeepPartial<MedusaeConfig> }) => {
...
export default function Medusae({ className, config, style }: { className?: string, config?: DeepPartial<MedusaeConfig>, style?: React.CSSProperties }) {
  const merged = useMemo(() => mergeConfig(config), [config]);
